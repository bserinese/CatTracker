...
#define GPS_UPDATE_TIMEOUT 3600000  //changed to 1 hour to give GPS time to lock on the first time
...
/* OTAA para*/
uint8_t devEui[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};  // devEui in ChirpStack
uint8_t appEui[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }; // JoinEUI in ChirpStack
uint8_t appKey[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }; //appKey in ChirpStack
...
uint32_t appTxDutyCycle = 120000;  //uploads new data every 2 minutes
...
void loop()
{
  switch( deviceState )
  {
    ...
    {
      LoRaWAN.displayAck();
      //LoRaWAN.sleep(); there seems to be a bug in the Sleep function that the forums haven't figured out so I just commented it out otherwise it stays asleep after a few uploads
      break;
    }
    default:
    {
